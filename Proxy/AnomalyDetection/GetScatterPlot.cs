...
{
    public static class GetScatterPlot
    {

        private static readonly HttpClient client = new HttpClient();

        [FunctionName("GetScatterPlot")]
            ...
            ILogger log)
        {
            ...
            var action_id = req.Query["actionid"];
            response = await client.PostAsync("https://irisanomalydetection.azurewebsites.net/api/GetScatterPlot?actionid=" + action_id, content);

            return (ActionResult) new FileStreamResult(await response.Content.ReadAsStreamAsync(), "image/png");
        }
